...
export const Reports: React.FC = () => {
  ...
  const generateReport = async () => {
    ...
    switch (activeReport) {
      case 'SALES':
        const sales = await StorageService.getSalesByPeriod(start.toISOString(), end.toISOString());
        setReportData(sales);
        setSummary({
      ...
      case 'CASH_CLOSING':
        ...
        const data = registers.map(r => {
          ...
          const salesTotal = txs.filter(t => t.type === 'SALE').reduce((a, b) => a + b.amount, 0);
          return { ...r, sales: salesTotal };
        });
        setReportData(data);
        break;
