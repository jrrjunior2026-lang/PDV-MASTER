    ...
    const loadData = async () => {
      try {
        ...
        if (salesData.length > 0 || productsData.length > 0) {
          GeminiService.analyzeBusiness(salesData, productsData).then(setInsight);
        } else {
          setInsight("Bem-vindo ao PDV Master! Comece cadastrando produtos e fazendo vendas para ver análises inteligentes aqui.");
        }
      } catch (error) {
        console.error('Error loading dashboard data:', error);
        setSales([]);
        setProducts([]);
        setInsight("Erro ao carregar dados. Verifique sua conexão com o Supabase.");
      } finally {
        setLoading(false);
      }
    };
